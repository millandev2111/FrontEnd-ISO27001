    ...
    const fetchAuditorias = async () => {
        try {
            ...
            if (axios.isAxiosError(err)) {
                ...
                setError('Error desconocido al cargar las auditorías')
            }
        } finally {
            setLoading(false)
        }
                                ...
                                >
                                    <div className="p-6">
                                        ...
                                        {auditoria.controladors && (
                                            <div className="mb-4">
                                                <span className="text-sm text-gray-600">
                                                    {auditoria.controladors.length} controles asignados
                                                </span>
                                            </div>
                                        )}
                                        ...
                                        <div className="flex justify-between items-center">
                                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(auditoria.state || 'pending')}`}>
                ...
                {!loading && !error && filteredAuditorias.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-600 mb-4">
                            {searchTerm ? 'No se encontraron auditorías con ese término de búsqueda' : 'Comienza creando tu primera auditoría'}
                        </p>
                        <button
                            onClick={() => setShowCreateModal(true)}

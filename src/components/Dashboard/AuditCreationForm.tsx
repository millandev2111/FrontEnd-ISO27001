// @/hooks/useAuditoriaResultados.ts - Versión simplificada
  ...
  const cargarDatos = useCallback(async (forzarRecarga = false) => {
    ...
    try {
      ...
      if (controles.length === 0 || forzarRecarga) {
        try {
          const responseControles = await axios.get(`${API_BASE}/controladors?populate=*`, {
            headers: { Authorization: `Bearer ${token}` },
            params: {
              'populate': '*', // Incluir también populate=* en controles
              'pagination[pageSize]': 200
            }
          });

          console.log('Respuesta completa de controles:', responseControles.data);

          if (responseControles.data && responseControles.data.data) {
            const controlesData = responseControles.data.data.map((item: any) => {
              // Mostramos la estructura completa del control para depuración
              console.log(`Control ID ${item.id} estructura completa:`, JSON.stringify(item, null, 2));

              // Normalizar estructura básica para controles
              const attributes = item.attributes || {};

              // Revisar todas las propiedades disponibles
              console.log(`Control ${item.id} attributes keys:`, Object.keys(attributes));
              ...
              console.log(`Tipo "${tipo}" mapeado a dominio: "${domain}" para control ${code}`);
              ...
              const control = {
                ...
                domain: domain // Usar el nombre mapeado del dominio
              };

              console.log(`Control procesado: ID=${control.id}, Code=${control.code}, Name=${control.name}, Domain=${control.domain}`);
      ...
      if (responseResultados.data && responseResultados.data.data) {
        ...
        const resultadosData = responseResultados.data.data.map((item: any) => {
          ...
          console.log(`Procesando resultado ID: ${rawItem.id}`);
          console.log(`Datos originales:`, JSON.stringify(rawItem, null, 2));
          ...
          if (rawItem.auditoria) {
            console.log(`Resultado ${rawItem.id} tiene auditoría:`, JSON.stringify(rawItem.auditoria, null, 2));
          ...
          console.log(`Buscando control para resultado ${rawItem.id}:`);
          console.log(`- controladorDirecto:`, controladorDirecto ? 'SI' : 'NO');
          console.log(`- controladorData:`, controladorData ? 'SI' : 'NO');
          console.log(`- controladorRelacion:`, controladorRelacion ? 'SI' : 'NO');

          // Primero intentar con controlador directo
          if (controladorDirecto) {
            // Código existente para controlador directo
            console.log(`Resultado ${rawItem.id} tiene controlador directo:`, JSON.stringify(controladorDirecto, null, 2));
            ...
            // Imprimir todas las propiedades para debugging
            console.log(`Propiedades del control en controlador directo:`, Object.keys(controlRaw));
            if (controlAttributes) console.log(`Propiedades de attributes:`, Object.keys(controlAttributes));
            ...
            console.log(`Control procesado del resultado: ID=${controlData.id}, Code=${controlData.code}, Name=${controlData.name}`);
          }
          // Probar con la estructura control.data (formato Strapi común)
          else if (controladorData) {
            console.log(`Resultado ${rawItem.id} tiene control.data:`, JSON.stringify(controladorData, null, 2));
            ...
            controlId = controlData.documentId;
            console.log(`Control procesado desde control.data: ID=${controlData.id}, Code=${controlData.code}, Name=${controlData.name}`);
          }
          // Probar con otros formatos de relación
          else if (controladorRelacion) {
            console.log(`Resultado ${rawItem.id} tiene control en otro formato:`, JSON.stringify(controladorRelacion, null, 2));
            // Intentar extraer información del control de la estructura
          }
          // Buscar por controlId si está disponible
            ...
            console.log(`Buscando control con ID ${controlIdToSearch} en ${controles.length} controles`);
            ...
            if (controlPorId) {
              console.log(`Encontrado control por ID numérico: ${controlPorId.code}`);
              controlData = controlPorId;
              controlId = controlPorId.documentId;
            } else if (controlPorDocumentId) {
              console.log(`Encontrado control por documentId: ${controlPorDocumentId.code}`);
              controlData = controlPorDocumentId;
              controlId = controlPorDocumentId.documentId;
            } else {
              console.log(`No se encontró control para el ID ${controlIdToSearch}`);
            }
          }
          ...
          if (!controlData) {
            ...
            const searchForControlFields = (obj: any, path = ''): Control | null => {
              ...
              if (obj.code && (obj.name || obj.title || obj.ask)) {
                // ... (mapeo dominio y creación de objeto control)
            ...
            if (foundControl) {
              console.log(`Control encontrado mediante búsqueda profunda: ${foundControl.code}`);
              controlData = foundControl;
              controlId = foundControl.documentId;
            } else {
              console.log(`⚠️ No se encontró control asociado para el resultado ${rawItem.id}`);

              // Intentar usar el campo A.6.1 que vimos en la imagen
              const fixedControl = controles.find(c => c.code === 'A.6.1');

              if (fixedControl) {
                console.log(`Usando control específico A.6.1 como último recurso`);
                controlData = fixedControl;
                controlId = fixedControl.documentId;
              } else if (controles.length > 0) {
                console.log(`⚠️ Usando primer control disponible como último recurso: ${controles[0].code}`);
                controlData = controles[0];
                controlId = controles[0].documentId;
              } else {
                console.log(`⚠️ No hay controles disponibles, creando uno genérico`);
                controlData = {
                  id: -1,
